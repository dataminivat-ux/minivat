-- =====================================================================
-- MINI VAT PREMIUM — Migration inicial (0001)
-- Fonte da verdade: docs/05-DATABASE-SCHEMA.md (NAO modificar o SQL)
-- Ordem: extensoes -> enums -> tabelas -> triggers/funcoes -> RLS -> views
-- =====================================================================
...
create type order_status as enum (
  'pending',        -- recem-criado, aguardando pagamento
  'paid',           -- pagamento aprovado
  'processing',     -- em separacao/embalagem
  'shipped',        -- despachado
  'delivered',      -- entregue
  'cancelled',      -- cancelado
  'refunded'        -- estornado
...
create table public.addresses (
  ...
  cep text not null,              -- so digitos, 8 chars
...
create table public.products (
  ...
  -- estoque agregado (variantes tem o proprio)
  stock integer not null default 0 check (stock >= 0),
  low_stock_threshold integer not null default 5,

  -- dimensoes pra calculo de frete (cm e g)
...
create table public.product_variants (
  ...
  price_cents integer check (price_cents >= 0),  -- override do preco base
  stock integer not null default 0 check (stock >= 0),
...
create table public.orders (
  ...
  -- valores em centavos (snapshot — nunca recalcular apos criar)
  ...
  shipping_service_id text,        -- ID do servico no Melhor Envio
  shipping_estimated_days integer,

  -- enderecos (snapshot completo)
...
-- 3.17 site_settings (+ seeds iniciais)
...
insert into public.site_settings (key, value, description) values
  ('free_shipping_threshold_cents', '20000', 'Valor minimo para frete gratis (200 BRL)'),
  ('max_installments', '6', 'Parcelas maximas no cartao'),
  ('checkout_min_total_cents', '5000', 'Compra minima (50 BRL)'),
...
-- ---------------------------------------------------------------------
-- 4. Triggers e funcoes utilitarias
-- ---------------------------------------------------------------------

-- 4.1 updated_at automatico
...
-- 4.2 numeracao de pedidos MVP-YYYY-NNNNNN
...
-- 4.3 historico automatico de status do pedido
...
-- catalogo (publico pode ler ativos)
...
-- orders: cliente ve os proprios; admin ve tudo. Inserts vao sempre via API server (service role).
...
-- inserts NUNCA pelo cliente; so service role.
